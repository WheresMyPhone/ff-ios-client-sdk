...
class CFConfigurationBuilderTest: XCTestCase {
    ...
    func testBuild() {
        ...
        XCTAssertTrue(config.baseUrl == ConfigurationMocks.baseUrl &&
                      config.streamUrl == ConfigurationMocks.streamUrl)
    }
    ...
    func testAttachStreamUrl() {
        let streamURL = ConfigurationMocks.streamUrl
        sut = sut.setStreamUrl(streamURL)
        let config = sut.build()
        XCTAssert(config.streamUrl == streamURL)
